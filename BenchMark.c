...
void benchmarkBubbleSort(int *vet_tam, int *vet_semente) {
	...
	printf("%-80s %s\n\n","","Tempo (ms)");
	printf("%-30s %-25s %-25s %-25s %-25s %-25s\n", "", "Tamanho", "Aleatório", "Ordem", "Ordem", "Quase");
	printf("%-30s %-25s %-25s %-25s %-25s %-25s\n", "Teste", "Entrada", "(média de 5)", "crescente", "decrescente", "ordenado");
	printf("%-30s %-25s %-25s %-25s %-24s %-25s\n\n", "", "", "", "", "", "(média de 5)");
	...
	for(int i=0;i<5;i++){
		...
		//Imprime os valores obtidos da entrada
  		printf("%-30d %-25d %-25.2f %-25.2f %-25.2f %-25.2f\n", i+1, vet_tam[i], vet_tempo[0], vet_tempo[1], vet_tempo[2], vet_tempo[3]);   
  		free(vet_tempo);
  		free(vet_trocas);
  		free(vet_comp);
